...
int main() {
    ...
    // по адресу 0 в буфере будем хранить курсор для писателя
    buffer->store[0] = 2;
    // по адресу 1 в буфере будем хранить курсор для читателя
    buffer->store[1] = 2;
    ...
    while (1) {
        ...
        int i = buffer->store[0]; // выгружаем курсор
        while(buffer->store[i] != -1) {
            printf("    --> checking store: store[%d] = %d\n", i, buffer->store[i]);
            ++i;
            if (i == BUF_SIZE) { // зацикливаем обход буфера
                i = 2;
            }
        }
        ...
        buffer->store[0] = i; // сохраняем новое значение курсора
